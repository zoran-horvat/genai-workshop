...
public class CompaniesRepository : IRepository<Company>
{
    ...
    {
        const string sql = @"SELECT c.Id, c.ExternalId, c.Name, c.TIN, c.Deleted, a.Id, a.ExternalId, a.StreetAddress, a.City, a.State, a.PostalCode, a.Country
    ...
    private record CompanyDto(int Id, Guid ExternalId, string Name, string TIN, bool Deleted)
    {
        public Company ToModel(AddressDto address) =>
            CompanyFactory.CreateExisting(new(ExternalId), Name, TIN, address.ToModel());
    }

    private record AddressDto(int Id, Guid ExternalId, string StreetAddress, string City, string State, string PostalCode, string Country)
    {
        public Address ToModel() =>
            new Address(new(ExternalId), StreetAddress, City, State, PostalCode, Country);
    }

    public async Task<Company> AddAsync(Company entity, CancellationToken cancellationToken = default)
    {
        const string insertCompany = @"INSERT INTO business.Companies (Name, TIN, UserId, Deleted, ExternalId)
                                       OUTPUT INSERTED.Id
                                       VALUES (@Name, @TIN, @UserId, 0, @ExternalId);";
        var companyId = await _connection.ExecuteScalarAsync<int>(
            insertCompany,
            new { entity.Name, entity.TIN, UserId = _userId.Value, ExternalId = entity.ExternalId.Value },
            transaction: _transaction
        );
    ...
    {
        const string insertAddress = @"INSERT INTO business.Addresses (CompanyId, StreetAddress, City, State, PostalCode, Country, ExternalId)
                                       OUTPUT INSERTED.Id
                                       VALUES (@CompanyId, @StreetAddress, @City, @State, @PostalCode, @Country, @ExternalId);";
    ...
    public async Task UpdateAsync(Company entity, CancellationToken cancellationToken = default)
    {
        ...
        const string updateCompany = @"UPDATE business.Companies
                                       ...
                                       WHERE ExternalId = @ExternalId AND UserId = @UserId AND Deleted = 0;";
        var companyRows = await _connection.ExecuteAsync(
            updateCompany,
            new { entity.Name, entity.TIN, Externalid = entity.ExternalId.Value, UserId = _userId.Value },
            transaction: _transaction
        );
    ...
    private async Task UpdateAddress(Company entity)
    {
        const string updateAddress = @"UPDATE business.Addresses
                                       SET StreetAddress = @StreetAddress, City = @City,
                                           State = @State, PostalCode = @PostalCode, Country = @Country, ExternalId = @ExternalId
